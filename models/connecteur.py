...
code_gra = ['02', '07', '19', '27', '28', '30', '74', '77', '80', '85', '86', '93']
code_sa_old = ['01', '06', '15', '17', '22', '29', '38', '42', '43', '45', '51',
               '52', '55', '60', '71', '83', '87']
code_sa = [
...
# sous_agence : agence
map_code_sa_old = {
    '01': '18', '04': '11', '05': '11', '06': '11',
    '09': '99', '15': '11', '17': '11',
    '38': '32', '42': '11', '43': '40', '45': '40',
    '51': '50', '52': '50', '71': '70', '87': '40',
    '55': '53', '29': '20', '22': '20', '83': '20',
    '60': '50',
}
map_code_sa = {
...
class connecteur(models.Model):
    ...
    def archivate_incorrect_data(self, filename, fieldnames, row):
        ...
        path = '/opt/cptodoo/filestore/' + filename
        _logger.info('\n=== row 1 = %s ===\n' % row)
        if row:
            row = {k:v.encode('utf-8') if isinstance(v, basestring) else v for k,v in row.items()}
        _logger.info('\n=== row 2 = %s ===\n' % row)
    ...
    def dispatch_mapped_data(self, data):
        ...
        if data.get('serial_identification_a') != '000':
            res[el_req[2]] = {
                ...
                'account_amount': data.get('code_app_a'),
            }
        ...
        res[el_req[5]] = {
            ...
            'access_amount': float(data.get('access_amount')),
            'tax_te': float(data.get('tax_te'))
        }
        ...
        res[el_req[6]] = {
            ...
            'access_amount': float(data.get('access_amount')),
            'tax_te': float(data.get('tax_te')),
            'tax_tva': float(data.get('tax_tva'))
        }
        res[el_req[7]] = {}
        res[el_req[8]] = {}
        ...
        access_2 = {'product_id': accessories,
                    'price_unit': float(data.get('access_amount'))}
        ...
        date_invoice = datetime.datetime.strptime(date_invoice, '%d/%m/%Y')
        res[el_req[10]] = {
        ...
        if data.get('agency_id') in code_gra:
            ...
            res[el_req[12]] = {
                'agency_id': data.get('agency_id'),
                'account_amount': data.get('commag_amount'),
    ...
    def fix_tax_difference(self, invoice, taxes):
        ...
        for tax in invoice.tax_line:
            ...
            elif tax.name.startswith('Te') or tax.name.startswith('TE'):
    ...
    def update_add_record(self):
        ...
        prm = "DRIVER=%s;SERVER=%s;PORT=%s;DATABASE=%s;UID=%s;PWD=%s;TDS_Version=%s" % (DRIVER, SERVER, PORT, DATABASE, UID, PWD, TDS_Version)
        try:
            ...
            cursorSQLServer.execute(request_sql3)
        except Exception, e:
            raise exceptions.Warning(_('Error'),
        ...
        for dpt_datas in dpt_datas_all:
            ...
            if dpt_datas.get('partner_ag', False):
                ...
                apt_id = self.check_partner_apporteur(cursorSQLServer, vals)
            ...
            # ttl = len(invoice_all)
            # ttct = 0
            # for invoice_id in invoice_all:
            # inv_st = timeit.default_timer()
            """
            # recompute_taxe
            invoice_id.button_reset_taxes()
            """
    ...
    @api.multi
    def patch_partner(self, cursorSQLServer, dpt_datas, cust_acc, part_cust, com_list, invoice_vals, px_ctg):
        if dpt_datas.get('partner_ag', False):
            # _logger.info('=== p_ag===')
            p_ag = dpt_datas.get('partner_ag')
            agency_code = p_ag.get('agency_id')
